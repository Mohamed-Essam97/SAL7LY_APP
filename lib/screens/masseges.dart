...
class _ChatState extends State<Chat> {
  ...
  Future<void> add() async {
    ...
    if (messageController.text.length > 0) {
      await _firestore.collection('chat').add({
        'service_owner': widget.user,
        'messege': massee,
        'client': widget.serivce_owner,
      });
  ...
  Future<void> callback() async {
    ...
    await Firestore.instance
        .collection('chat')
        .where("service_owner", isEqualTo: widget.user)
        .where("client", isEqualTo: widget.serivce_owner)
        .snapshots()
        .listen((onData) {
  ...
  Future _getImage() async {
    ...
    //.where("service_owner", isEqualTo: widget.serivce_owner)
      ...
      onData.documents.forEach((docc) {
        if(docc["client"]==widget.serivce_owner) {
          massee = List.from(docc["messege"]);
          // print(widget.serivce_owner.toString());
